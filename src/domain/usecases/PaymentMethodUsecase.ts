...
export class PaymentMethodUsecase {
	...
	async createPaymentMethod(
		...
		if (error) {
			throw new Error(
				`Invalid CreatePaymentMethodRequest data: ${error.message}`,
			);
		}
	...
	async updatePaymentMethod(
		id: number,
		paymentMethodData: UpdatePaymentMethodRequest,
	): Promise<PaymentMethod | null> {
		// Validate paymentMethodData against the schema
		const { error } = updatePaymentMethodValidation.validate({
			...paymentMethodData,
		});
		if (error) {
			throw new Error(
				`Invalid UpdatePaymentMethodRequest data: ${error.message}`,
			);
		}
