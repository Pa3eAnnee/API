...
import {
	type CreateParticipateRequest,
	type UpdateParticipateRequest,
	createParticipateValidation,
	updateParticipateValidation,
} from "../../handlers/validators/participate-validator";
...
export class ParticipateUsecase {
	...
	async updateParticipate(
		id: number,
		participateData: UpdateParticipateRequest,
	): Promise<Participate | null> {
		// Validate participateData against the schema
		const { error } = updateParticipateValidation.validate(participateData);
		if (error) {
			throw new Error(
				`Invalid UpdateParticipateRequest data: ${error.message}`,
			);
		}
