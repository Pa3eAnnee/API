...
export class SubscriptionUsecase {
	...
	async createSubscription(
		...
		if (error) {
			throw new Error(
				`Invalid CreateSubscriptionRequest data: ${error.message}`,
			);
		}
	...
	async updateSubscription(
		...
		// Validate subscriptionData against the schema
		const { error } = updateSubscriptionValidation.validate({
			...subscriptionData,
		});
		if (error) {
			throw new Error(
				`Invalid UpdateSubscriptionRequest data: ${error.message}`,
			);
		}
