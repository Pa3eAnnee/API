...
import {
	type CreateMembershipCardRequest,
	type UpdateMembershipCardRequest,
	createMembershipCardValidation,
	updateMembershipCardValidation,
...
export class MembershipCardUsecase {
	...
	async createMembershipCard(
		membershipCardData: CreateMembershipCardRequest,
	): Promise<MembershipCard> {
		// Validate membershipCardData against the schema
		const { error } =
			createMembershipCardValidation.validate(membershipCardData);
		if (error) {
			throw new Error(
				`Invalid CreateMembershipCardRequest data: ${error.message}`,
			);
		}
	...
	async updateMembershipCard(
		id: number,
		membershipCardData: UpdateMembershipCardRequest,
	): Promise<MembershipCard | null> {
		// Validate membershipCardData against the schema
		const { error } = updateMembershipCardValidation.validate({
			...membershipCardData,
		});
		if (error) {
			throw new Error(
				`Invalid UpdateMembershipCardRequest data: ${error.message}`,
			);
		}
