...
import {
	type CreateGeneralAssemblyRequest,
	type UpdateGeneralAssemblyRequest,
	createGeneralAssemblyValidation,
	updateGeneralAssemblyValidation,
...
export class GeneralAssemblyUsecase {
	...
	async createGeneralAssembly(
		generalAssemblyData: CreateGeneralAssemblyRequest,
	): Promise<GeneralAssembly> {
		// Validate generalAssemblyData against the schema
		const { error } =
			createGeneralAssemblyValidation.validate(generalAssemblyData);
		if (error) {
			throw new Error(
				`Invalid CreateGeneralAssemblyRequest data: ${error.message}`,
			);
		}
	...
	async updateGeneralAssembly(
		id: number,
		generalAssemblyData: UpdateGeneralAssemblyRequest,
	): Promise<GeneralAssembly | null> {
		// Validate generalAssemblyData against the schema
		const { error } = updateGeneralAssemblyValidation.validate({
			...generalAssemblyData,
		});
		if (error) {
			throw new Error(
				`Invalid UpdateGeneralAssemblyRequest data: ${error.message}`,
			);
		}
