...
import {
    CreateExpanseCategoryRequest,
    createExpanseCategoryValidation, UpdateExpanseCategoryRequest, updateExpanseCategoryValidation
...
export class ExpanseCategoryUsecase {
    ...
    async createExpanseCategory(expanseCategoryData: CreateExpanseCategoryRequest): Promise<ExpanseCategory> {
        // Validate expanseCategoryData against the schema
        const { error } = createExpanseCategoryValidation.validate(expanseCategoryData);
        if (error) {
            throw new Error(`Invalid CreateExpanseCategoryRequest data: ${error.message}`);
        }
    ...
    async updateExpanseCategory(id: number, expanseCategoryData: UpdateExpanseCategoryRequest): Promise<ExpanseCategory | null> {
        // Validate expanseCategoryData against the schema
        const { error } = updateExpanseCategoryValidation.validate({ ...expanseCategoryData });
        if (error) {
            throw new Error(`Invalid UpdateExpanseCategoryRequest data: ${error.message}`);
        }
