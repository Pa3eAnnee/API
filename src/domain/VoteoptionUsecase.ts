...
import {
	type CreateVoteOptionRequest,
	type UpdateVoteOptionRequest,
	createVoteOptionValidation,
	updateVoteOptionValidation,
...
export class VoteOptionUsecase {
	...
	async createVoteOption(
		voteOptionData: CreateVoteOptionRequest,
	): Promise<VoteOption> {
	...
	async updateVoteOption(
		id: number,
		voteOptionData: UpdateVoteOptionRequest,
	): Promise<VoteOption | null> {
		// Validate voteOptionData against the schema
		const { error } = updateVoteOptionValidation.validate({
			...voteOptionData,
		});
		if (error) {
