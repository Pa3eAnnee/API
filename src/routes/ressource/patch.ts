...
export const updateRessource = (app: Express): void => {
	app.patch("/ressources/:id", async (req: Request, res: Response) => {
		...
		const validation = updateRessourceValidation.validate({
			...req.body,
		...
		try {
			const Ressource = await RessourceRepo.findOneBy({
				id: RessourceRequest.id,
			});
			if (!Ressource) {
				res.status(404).send({
					error: `Ressource with ID ${RessourceRequest.id} not found`,
				});
				return;
			}

			await RessourceRepo.update(RessourceRequest.id, RessourceRequest);
			const updatedRessource = await RessourceRepo.findOneBy({
				id: RessourceRequest.id,
			});
