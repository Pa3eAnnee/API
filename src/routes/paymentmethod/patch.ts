...
export const updatePaymentMethod = (app: Express): void => {
	app.patch("/paymentmethods/:id", async (req: Request, res: Response) => {
		...
		const validation = updatePaymentMethodValidation.validate({
			...req.body,
		...
		try {
			const paymentMethod = await paymentMethodRepo.findOneBy({
			...
			const updatedPaymentMethod = await paymentMethodRepo.findOneBy({
				id: paymentMethodRequest.id,
			});
