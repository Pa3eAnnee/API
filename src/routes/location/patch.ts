...
export const updateLocation = (app: Express): void => {
	app.patch("/locations/:id", async (req: Request, res: Response) => {
		...
		const validation = updateLocationValidation.validate({
			...req.body,
		...
		try {
			...
			if (!Location) {
				res
					.status(404)
					.send({ error: `Location with ID ${LocationRequest.id} not found` });
				return;
			}

			await LocationRepo.update(LocationRequest.id, LocationRequest);
			const updatedLocation = await LocationRepo.findOneBy({
