...
export const updateActivity = (app: Express): void => {
    app.patch('/activities/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateActivityValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            ...
            if (!Activity) {
                res.status(404).send({ error: `Activity with ID ${ActivityRequest.id} not found` });
                return;
            }

            await ActivityRepo.update(ActivityRequest.id, ActivityRequest);
            const updatedActivity = await ActivityRepo.findOneBy({ id: ActivityRequest.id });
