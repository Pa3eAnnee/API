...
export const updateRessourceCategory = (app: Express): void => {
    app.patch('/ressourcecategories/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateRessourceCategoryValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            const RessourceCategory = await RessourceCategoryRepo.findOneBy({ id: RessourceCategoryRequest.id });
            if (!RessourceCategory) {
                res.status(404).send({ error: `RessourceCategory with ID ${RessourceCategoryRequest.id} not found` });
                return;
            }

            await RessourceCategoryRepo.update(RessourceCategoryRequest.id, RessourceCategoryRequest);
            const updatedRessourceCategory = await RessourceCategoryRepo.findOneBy({ id: RessourceCategoryRequest.id });
