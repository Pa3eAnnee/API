...
export const deleteRessourceCategory = (app: Express): void => {
    app.delete('/ressourcecategories/:id', async (req: Request, res: Response) => {
        const RessourceCategoryId = parseInt(req.params.id);

        if (isNaN(RessourceCategoryId)) {
        ...
        try {
            ...
            res.status(200).send({ message: `RessourceCategory with ID ${RessourceCategoryId} deleted successfully` });
        } catch (error) {
            console.error(error);
