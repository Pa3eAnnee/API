...
export const updateVoteoption = (app: Express): void => {
    app.patch('/voteoptions/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateVoteOptionValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            const voteoption = await voteoptionRepo.findOneBy({ id: voteoptionRequest.id });
            if (!voteoption) {
                res.status(404).send({ error: `Voteoption with ID ${voteoptionRequest.id} not found` });
                return;
            }

            await voteoptionRepo.update(voteoptionRequest.id, voteoptionRequest);
            const updatedVoteoption = await voteoptionRepo.findOneBy({ id: voteoptionRequest.id });
