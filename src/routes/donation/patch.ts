...
export const updateDonation = (app: Express): void => {
    app.patch('/donations/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateDonationValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            ...
            if (!donation) {
                res.status(404).send({ error: `Donation with ID ${donationRequest.id} not found` });
                return;
            }

            await donationRepo.update(donationRequest.id, donationRequest);
            const updatedDonation = await donationRepo.findOneBy({ id: donationRequest.id });
