...
export const updateAgenda = (app: Express): void => {
    app.patch('/agendas/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateAgendaValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            ...
            if (!agenda) {
                res.status(404).send({ error: `Agenda with ID ${agendaRequest.id} not found` });
                return;
            }

            await agendaRepo.update(agendaRequest.id, agendaRequest);
            const updatedAgenda = await agendaRepo.findOneBy({ id: agendaRequest.id });
