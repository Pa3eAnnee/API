...
export const updateTransaction = (app: Express): void => {
    app.patch('/transactions/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateTransactionValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            const transaction = await transactionRepo.findOneBy({ id: transactionRequest.id });
            if (!transaction) {
                res.status(404).send({ error: `Transaction with ID ${transactionRequest.id} not found` });
                return;
            }

            await transactionRepo.update(transactionRequest.id, transactionRequest);
            const updatedTransaction = await transactionRepo.findOneBy({ id: transactionRequest.id });
