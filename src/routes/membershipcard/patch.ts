...
export const updateMembershipCard = (app: Express): void => {
	app.patch("/membershipcards/:id", async (req: Request, res: Response) => {
		...
		const validation = updateMembershipCardValidation.validate({
			...req.body,
		...
		try {
			...
			await membershipCardRepo.update(
				membershipCardRequest.id,
				membershipCardRequest,
			);
			const updatedMembershipCard = await membershipCardRepo.findOneBy({
				id: membershipCardRequest.id,
			});
