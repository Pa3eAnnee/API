...
export const updateMeeting = (app: Express): void => {
    app.patch('/meetings/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateMeetingValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            ...
            if (!Meeting) {
                res.status(404).send({ error: `Meeting with ID ${MeetingRequest.id} not found` });
                return;
            }

            await MeetingRepo.update(MeetingRequest.id, MeetingRequest);
            const updatedMeeting = await MeetingRepo.findOneBy({ id: MeetingRequest.id });
