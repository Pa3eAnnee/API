...
export const updateFurnitureCategory = (app: Express): void => {
	app.patch("/furniturecategorys/:id", async (req: Request, res: Response) => {
		...
		const validation = updateFurnitureCategoryValidation.validate({
			...req.body,
		...
		try {
			...
			await furnitureCategoryRepo.update(
				furnitureCategoryRequest.id,
				furnitureCategoryRequest,
			);
			const updatedFurnitureCategory = await furnitureCategoryRepo.findOneBy({
				id: furnitureCategoryRequest.id,
			});
