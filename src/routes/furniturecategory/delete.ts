...
export const deleteFurnitureCategory = (app: Express): void => {
    app.delete('/furniturecategorys/:id', async (req: Request, res: Response) => {
        const furnitureCategoryId = parseInt(req.params.id);

        if (isNaN(furnitureCategoryId)) {
        ...
        try {
            const furnitureCategory = await furnitureCategoryRepo.findOneBy({ id: furnitureCategoryId });
            if (!furnitureCategory) {
                res.status(404).send({ error: `FurnitureCategory with ID ${furnitureCategoryId} not found` });
                return;
            }

            await furnitureCategoryRepo.delete(furnitureCategoryId);
            res.status(200).send({ message: `FurnitureCategory with ID ${furnitureCategoryId} deleted successfully` });
        } catch (error) {
            console.error(error);
