...
export const updateGeneralAssembly = (app: Express): void => {
    app.patch('/generalassemblies/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateGeneralAssemblyValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            const generalAssembly = await generalAssemblyRepo.findOneBy({ id: generalAssemblyRequest.id });
            if (!generalAssembly) {
                res.status(404).send({ error: `GeneralAssembly with ID ${generalAssemblyRequest.id} not found` });
                return;
            }

            await generalAssemblyRepo.update(generalAssemblyRequest.id, generalAssemblyRequest);
            const updatedGeneralAssembly = await generalAssemblyRepo.findOneBy({ id: generalAssemblyRequest.id });
