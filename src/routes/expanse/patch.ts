...
export const updateExpanse = (app: Express): void => {
    app.patch('/expanses/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateExpanseValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            ...
            if (!expanse) {
                res.status(404).send({ error: `Expanse with ID ${expanseRequest.id} not found` });
                return;
            }

            await expanseRepo.update(expanseRequest.id, expanseRequest);
            const updatedExpanse = await expanseRepo.findOneBy({ id: expanseRequest.id });
