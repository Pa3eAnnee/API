...
export const deleteSubscription = (app: Express): void => {
    app.delete('/subscriptions/:id', async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (!subscription) {
                res.status(404).send({ error: `Subscription with ID ${subscriptionId} not found` });
                return;
            }

            await subscriptionRepo.delete(subscriptionId);
            res.status(200).send({ message: `Subscription with ID ${subscriptionId} deleted successfully` });
        } catch (error) {
            console.error(error);
