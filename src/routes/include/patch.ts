...
export const updateInclude = (app: Express): void => {
	app.patch("/includes/:id", async (req: Request, res: Response) => {
		...
		const validation = updateIncludesValidation.validate({
			...req.body,
		...
		try {
			...
			if (!include) {
				res
					.status(404)
					.send({ error: `Include with ID ${includeRequest.id} not found` });
				return;
			}

			await includeRepo.update(includeRequest.id, includeRequest);
			const updatedInclude = await includeRepo.findOneBy({
				id: includeRequest.id,
			});
