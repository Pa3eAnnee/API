...
export const updateDebate = (app: Express): void => {
    app.patch('/debates/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateDebateValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            ...
            if (!debate) {
                res.status(404).send({ error: `Debate with ID ${debateRequest.id} not found` });
                return;
            }

            await debateRepo.update(debateRequest.id, debateRequest);
            const updatedDebate = await debateRepo.findOneBy({ id: debateRequest.id });
