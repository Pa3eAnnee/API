...
export const updateExpanseCategory = (app: Express): void => {
	app.patch("/expansecategories/:id", async (req: Request, res: Response) => {
		...
		const validation = updateExpanseCategoryValidation.validate({
			...req.body,
		...
		try {
			const expanseCategory = await expanseCategoryRepo.findOneBy({
			...
			await expanseCategoryRepo.update(
				expanseCategoryRequest.id,
				expanseCategoryRequest,
			);
			const updatedExpanseCategory = await expanseCategoryRepo.findOneBy({
				id: expanseCategoryRequest.id,
			});
