...
export const updateTopics = (app: Express): void => {
    app.patch('/topics/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateTopicsValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            ...
            if (!topics) {
                res.status(404).send({ error: `Topics with ID ${topicsRequest.id} not found` });
                return;
            }

            await topicsRepo.update(topicsRequest.id, topicsRequest);
            const updatedTopics = await topicsRepo.findOneBy({ id: topicsRequest.id });
