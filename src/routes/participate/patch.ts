...
export const updateParticipate = (app: Express): void => {
    app.patch('/participates/:id', async (req: Request, res: Response) => {
        ...
        const validation = updateParticipateValidation.validate({ ...req.body, id: parseInt(req.params.id) });
        ...
        try {
            const Participate = await ParticipateRepo.findOneBy({ id: ParticipateRequest.id });
            if (!Participate) {
                res.status(404).send({ error: `Participate with ID ${ParticipateRequest.id} not found` });
                return;
            }

            await ParticipateRepo.update(ParticipateRequest.id, ParticipateRequest);
            const updatedParticipate = await ParticipateRepo.findOneBy({ id: ParticipateRequest.id });
